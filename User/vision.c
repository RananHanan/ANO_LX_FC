...
//参照ANO_LX_DT中接收代码仿写，相比于 MY_uart2_Recive 优化后的写法。
static u8 VS_RxBuffer[256], VS_data_cnt = 0;
void Vision_GetByte(u8 data)
{
  //长度只是被用来赋值后,方便数据区的数据接收，而cnt是用来接收数据的地址偏移
	...
	else if (rxstate == 6)
	{
		rxstate = 0;
		VS_RxBuffer[4 + _data_cnt] = data;
		VS_data_cnt = _data_cnt + 5;
		
		//接收完数据，准备对数据进行处理
		//VS_dt_data_ok = 1;
		Vision_Recevier_anl(VS_RxBuffer, VS_data_cnt);
		//DrvUart1SendBuf(VS_RxBuffer, VS_data_cnt);
...
void Vision_Recevier_anl(u8 *data, u8 len)
{
